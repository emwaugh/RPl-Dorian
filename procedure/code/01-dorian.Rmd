...
dorian_text <- dorian %>%
  select(text) %>%
  plain_tweets() #removes non-language elements, from rtweet

# parse out words from tweet text
dorian_words <- dorian_text %>% unnest_tokens(word, text) # creates one column with all individual words in all tweets as rows
...
#drawing lines between each county neighborhood, darker lines mean more counties in a neighborhood
...
ggplot() +
  ...
  labs(title = "Increased Discussion of Hurricanes on Twitter During Hurrican Dorian") +
  theme(
    plot.title = element_text(hjust = 0.5),
    axis.title.x = element_blank(),
    axis.title.y = element_blank()
...
```{r log-scale-dorrate}
# take log scale, when dorrate = 0, replace with 0.1 to make not NA
counties <- counties %>% 
...
ggplot() +
  ...
  labs(title = "Tweet Density During Hurricane Dorian") +
  theme(
    plot.title = element_text(hjust = 0.5),
    axis.title.x = element_blank(),
    axis.title.y = element_blank()
